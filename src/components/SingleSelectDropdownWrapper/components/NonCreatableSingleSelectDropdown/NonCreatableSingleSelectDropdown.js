...
import {MenuItem, NoSsr, Paper, TextField, Typography} from '@material-ui/core';
...
const useStyles = theme => ({
  ...
  singleValue: {color: '#212121', fontSize: '16px', fontFamily: 'Roboto', fontWeight: '400', marginLeft: '2px', marginRight: '2px',
    maxWidth: 'calc(100% - 32px)', overflow: 'hidden', position: 'absolute', textOverflow: 'ellipsis', whiteSpace: 'nowrap', top: '50%',
...
/**
 * @function to customised the indicator when disabled
 * @returns {null}
 * @constructor
 */
...
const DropdownIndicator = (
  ...
  return (
    <components.DropdownIndicator {...props}>
      <CustomSvgIcon icon={'dropdown-icon'} height="7.8px" width="14px"
                     color={props.isDisabled ? '#d3d3d3' : '#B1B1B1'}/>
...
class NonCreatableSingleSelectDropdown extends React.Component {
  state = {
    inputValue: '',
  ...
  /**
   * handling on blur event,updating state with selected value
   * @param e
   */
  handleBlur = (e) => {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    this.setState({selectedOption: nextProps.hasOwnProperty('defaultValue') ? nextProps.defaultValue : ''})
  }
...
NonCreatableSingleSelectDropdown.propTypes = {
  classes: PropTypes.object.isRequired,
  theme: PropTypes.object.isRequired,
};

export default withStyles(useStyles, {withTheme: true})(NonCreatableSingleSelectDropdown);
