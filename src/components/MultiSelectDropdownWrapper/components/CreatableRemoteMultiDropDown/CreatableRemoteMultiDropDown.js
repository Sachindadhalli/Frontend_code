...
import { Typography, NoSsr,  FormHelperText, TextField, Paper, MenuItem } from '@material-ui/core';
...
//utilities
import {apiCall, handleLocalStorage, debounce, calculateTotalCount} from '../../../../Utilities';
import keyToValue from '../../../../Utilities/ValueKeyConverstion';

//custom components
import CustomTag from "../../../CustomTag/CustomTag";
import ChipsForFields from '../../../../containers/EmployerHomePage/components/EmployerJobPage/components/SectorsRoles/ChipsForFields';
...
class CreatableRemoteMultiDropDown extends React.Component {
  state = {
    inputValue: '',
    selectedOption: '',
    options: [],
  ...
  handleChange = (selectedOption, {action}) => {
    if (selectedOption.length < (this.props.maxOptions + 1) || !this.props.maxOptions) {
      ...
      if (this.props.hasOwnProperty('gettingError')) {
  ...
  componentDidMount() {
    ...
    let valus = this.props.defaultValue ? this.props.defaultValue.map((item) => keyToValue(item)) : [];
    this.handleChange(valus, actionMetaData);
    this.setState({
      updatedSelectedValue: this.props.defaultValue,
      selectedOption: this.props.defaultValue,
      ShowkeySkills: this.props.ShowkeySkills
    })
  }

  componentWillReceiveProps(nextProps) {
    let valus = this.props.defaultValue ? nextProps.defaultValue.map((item) => keyToValue(item)) : [];
    this.setState({
      updatedSelectedValue: nextProps.defaultValue,
      errorValue: nextProps.error,
      maxSelected: nextProps.maxOptions,
      selectedOption: valus
    })
  }
      ...
      <div className={classes.root}>
        <NoSsr>
          ...
          <div style={{display: 'flex', width: '100%'}}>
            ...
            {this.state.ShowkeySkills ?
              <div className="key-skilles-footer" style={{width: '116px', marginTop: '9px'}}>
                <div className="character-info-section">
                  <CustomTag text="Character left : " className="character-left"/>
                  <CustomTag text={1000 - calculateTotalCount(this.state.updatedSelectedValue)} className="count"/>
                </div>
              </div> : null}
          </div>
          <ChipsForFields
...
CreatableRemoteMultiDropDown.propTypes = {
  classes: PropTypes.object.isRequired,
  theme: PropTypes.object.isRequired,
};

export default withStyles(useStyles, { withTheme: true })(CreatableRemoteMultiDropDown);
