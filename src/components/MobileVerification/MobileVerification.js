...
import {Button, Fab, FormControl, FormHelperText, Grid, Input, InputLabel, withStyles} from '@material-ui/core';
...
const styles = theme => ({
  ...
  disableButton: {backgroundImage: 'none', backgroundColor: '#656565',},
  input: {display: 'none',},
});
...
class MobileVerification extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps && nextProps.otpReceived) {
      this.setState({otpReceived: nextProps.otpReceived, is_received_otp: nextProps.otpReceived,});
    }
    if (nextProps && nextProps.otpVerified) {
      this.setState(
        {otpVerified: nextProps.otpVerified, otp_error: this.state.otp_error === ' ' ? ' ' : nextProps.otpVerified,},
        () => {
  ...
  /**
   * update given mobile number in the state
   * @param e
   */
  onInputChange = e => {
    const {name} = e.target;
    ...
    if (name === 'popup_mobile') {
      if (e.target.value.length === 10) this.setState({enableOTP: true});
      else this.setState({enableOTP: false});
    }
  };
   ...
   */
  enableGetOTPButton = () => {
    this.setState({is_get_otp_enable: true});
  };
  ...
  /**
   * @function to handle focus of otp input field
   * @param e
   */
  handlePressKey = e => {
  ...
  onOtpInputChange = e => {
    ...
    let {value} = e.target;
    this.setState(
      {[id]: !isNaN(e.target.value) && e.target.value !== '' ? e.target.value % 10 : e.target.value, otp_error: '',},
      () => {
        if ((this.state.input_four || this.state.input_four === "0") && (this.state.input_one || this.state.input_one === "0")
          && (this.state.input_two || this.state.input_two === "0") && (this.state.input_three || this.state.input_three === "0")) {
          this.setState({isSubmitEnable: true});
        }
        else this.setState({isSubmitEnable: false});
      },
  ...
  handleResendButton(newState, timeOut) {
    setTimeout(() => {
      this.setState({is_resend_enable: newState,});
    }, timeOut);
  };
  ...
  reSendOTP() {
    this.setState(
      ...
      () => {
        setTimeout(() => {
          this.setState({disableResend: false, leftTime: 45,});
          clearInterval(this.otpTimer);
        }, 45000);
      },
    );
  ...
  getOTP(e) {
    ...
    this.setState({popup_mobile_error: '',});
    clearInterval(this.otpTimer);
    this.setState(
      {
        enableMobile: false,
  ...
  render() {
    ...
    const {enableMobile, otp_error, countryCode, leftTime, otpVerified, popup_mobile_error,} = this.state;
    return (
                  ...
                  >
                    {this.state.isResendClicked ?
                      <Button color="secondary" className={classes.button}
                              ...
                              style={this.state.disableResend === true ? {
                                'cursor': 'not-allowed',
                                'color': 'gray',
                                width: '108px',
                                textTransform: 'none'
